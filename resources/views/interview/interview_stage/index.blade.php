...
@section('content')
    ...
    <section class="content">
                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            <table id="example1" class="DataTables table table-hover table-bordered table-striped">
                                ...
                                <tbody>
                                @foreach($interview_stages as $stage)
                                    <tr>
                                        <td>{{$stage->id}}</td>
                                        <td>{{$stage->title}}</td>
                                        <td>{{$stage->created_at->diffForHumans() }}</td>
                                        <td>
                                            ...
                                            {{--                                               data-toggle="modal" href="#edit{{$stage->id}}">تعديل<i class="las la-pen"></i></a>--}}
                                            {{--                                            <a class="modal-effect btn btn-sm btn-danger" data-effect="effect-scale"--}}
                                            {{--                                               data-toggle="modal" href="#delete{{$stage->id}}">حذف<i--}}
                                            {{--                                                    class="las la-trash"></i></a>--}}
                                            {{--                                            <a class="modal-effect btn btn-sm btn-danger" data-effect="effect-scale"--}}
                                            {{--                                               data-toggle="modal" href="{{route('interview_stage_item.index')}}">تفاصيل<i--}}
                                            {{--                                                    class="las la-trash"></i></a>--}}
                                        </td>
                                    </tr>
                                    @include('interview.interview_stage.modal.edit')
                                    @include('interview.interview_stage.modal.delete')
                                @endforeach
        ...
        @include('interview.interview_stage.modal.add')
    </section>
@endsection
