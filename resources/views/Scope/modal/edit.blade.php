<div class="modal fade" id="edit{{ $scope->id }}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form action="{{ route('scope.update', $scope->id) }}" method="post" autocomplete="off">
                    @method('PUT')
                    @csrf
                    <div class="form-group">
                        <input type="hidden" name="id" value="{{ $scope->id }}" class="form-control"
                               id="recipient-name">
                        <label for="recipient-name" class="col-form-label">اسم المجال</label>
                        <input type="text" name="name" value="{{ $scope->name }}" class="form-control"
...
{{--                    <div class="form-group">--}}
                        ...
                        <select name="department_id" class="form-control select2" style="width: 100%;">
                            <option selected="selected" value="{{$scope->department_id}}">{{$scope->department->name}}</option>
...
{{--            <form action="{{ route('scope.edit') }}" method="post" autocomplete="off">--}}
{{--                {{ method_field('patch') }}--}}
{{--                {{ csrf_field() }}--}}
{{--                @csrf--}}
{{--                <div class="modal-body">--}}
...
{{--                        <input type="text" name="name" value="{{ $scope->name }}" class="form-control">--}}
{{--                        <input type="text" name="name" value="{{ $scope->name }}" class="form-control" id="recipient-name">--}}
{{--                    </div>--}}
{{--                </div>--}}
